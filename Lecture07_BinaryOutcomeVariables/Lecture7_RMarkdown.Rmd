...
Suppose we are interested a causal model predicting which individuals purchase health insurance. We will model this purchase as a binary outcome, with $0 =$ no insurance, $1=$ insurance, and will use the same data from Lecture 5.

Note that you should have a project loaded for this lecture in order to access the data using `here()`.

Let's first estimate a linear probability model of insurance coverage as a function of enrollee age, sex, BMI, smoker status, and the number of children in the home. Let's estimate and interpret the LPM.
