...
#
# PURPOSE
#   Synthetic Controls and Quantile Regression
#
# NOTES: 
...
# Lecture 10 packages: synthetic controls
library(tidysynth)
library(gsynth)
library(gghighlight)
library(lubridate)
...
##### 1. Question: What was the effect of Ohio's vaccine lottery?  ####
# Replication of Lang et al, 2021: https://www.cambridge.org/core/journals/journal-of-experimental-political-science/article/did-ohios-vaccine-lottery-increase-vaccination-rates-a-preregistered-synthetic-control-study/07720E0BB974962FE4547FF2BCC71CAC#supplementary-materials
# Data available here: https://dataverse.harvard.edu/dataset.xhtml?persistentId=doi:10.7910/DVN/QYXN9L
...
# Switch back to slides here while we wait for this to run
...
# So what's the effect? Let's compare Ohio and Synthetic Ohio
...
# LAD Regression: What about the median spender (who is much cheaper than the average?)
library(quantreg)
...
msummary(list("OLS"=m_ols,"LAD"=m_lad),
         stars=c('*' = .1, '**' = .05, '***' = .01)) 
# Now, the effect of activity limitation is estimated to be $1578.93

# Quantile Regression: What about across the distribution of spending? 
...
# What does this look like if we ignore super spenders? 
...
# Scale these based on the sample quantile of spending
...
plotdata %>% mutate(coefs = coefs / myquants, 
  ...
  theme_classic() + labs(x="Quantile", y="Estimated Marginal Effect of Activity Limitation on Health Spending")
# Note: what's going on with the first one? Check myquants to find out! 
...
nprobust.plot(m1) #   xtitle="Log(Income)",ytitle="Health Expenitures") # not sure why this comman doesn't work
######################################
